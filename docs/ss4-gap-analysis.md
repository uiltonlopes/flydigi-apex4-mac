...
| 8 | **Macro library** — local macros, duplicate, rename, share/import by code | ✅ library (2026-09-02); share codes ❌ | We edit the macros stored *in the slot* only. SS4 also has a local library (`GetLocalMacroConfigList…`, share as decimal-dash byte string). | M |
...
| 10 | **Apply profile to NS (Switch) mode** | ✅ (2026-09-02) | `ApplySwitchConfig` → `SaveCurrentSwitchMappingConfig` (`A5 50 03` variant with the Switch slot). `IsSupportNs = true` for k2. Needs a protocol test on hardware (what the Switch slot id is). | M (research) |
